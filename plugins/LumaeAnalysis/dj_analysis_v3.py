...
from . import dj_contract as contract
from . import dj_evidence as evidence
from . import dj_runtime as runtime
from .vocal_calibration import CUT_RISK_THRESHOLD


SCHEMA_VERSION = 3
METHOD = "beat-this-1.1.0-yamnet-lite-1-lumae-dj-v3.1"
ANALYZER_VERSION = 1
...
def runtime_status(model_path, yamnet_model_path=None, **kwargs):
    ...
    status = json.loads(
        json.dumps(runtime.runtime_status(model_path, yamnet_model_path, **kwargs))
    )
    status.update(
        {
...
def _tempo_interpretation(raw_tempo, phase_gaps):
    ...
    if not 55.0 <= normalized <= 215.0:
        ...
        chosen = min(
            valid, key=lambda item: abs(item["tempo_bpm"] - 120.0), default=None
        )
        if chosen:
...
def _localized_regions(
    beat_frames, matched_downbeats, beat_logits, downbeat_logits, fps
):
    ...
    for segment_start, segment_end in zip(split_at, split_at[1:]):
        ...
        if length < 16:
            continue
        starts = list(
            range(segment_start, max(segment_start + 1, segment_end - 15), 32)
        )
        ...
        for start in sorted(set(starts)):
            ...
            mean_interval = (
                float(np.mean(local_intervals)) if len(local_intervals) else 0.0
            )
            cv = (
                float(np.std(local_intervals) / mean_interval)
                if mean_interval > 0
                else math.inf
            )
            local_downbeats = [
            ...
            for item in local_downbeats:
                ...
                confidence = evidence.sigmoid(downbeat_logits[item["raw_frame"]])
            ...
            aligned = (
                [
                    item
                    for item in local_downbeats
                    if item["beat_index"] % 4 == meter_phase
                ]
                if meter_phase is not None
                else []
            )
            expected = (
                sum(index % 4 == meter_phase for index in range(start, end))
                if meter_phase is not None
                else 0
            )
            ...
            raw_tempo = (
                60.0 / float(np.median(local_intervals))
                if len(local_intervals)
                else 0.0
            )
            phase_gaps = [
    ...
    return _cover_timeline(regions, MAX_REGIONS)


def _cover_timeline(values, limit):
    """Bound output without throwing away late-track evidence."""
    if len(values) <= limit:
        return values
...
def _vocal_risk_at(vocal_risk, position_ms):
    ...
    if abs(nearest["position_ms"] - position_ms) > 750:
        return None
...
def _speech_safe_intervals(model_output, vocal_risk, duration_ms):
    ...
    if not risky:
        return []
    low = evidence.finite_vector("low_energy", model_output.get("low_energy"))
    mid = evidence.finite_vector("mid_energy", model_output.get("mid_energy"), len(low))
    high = evidence.finite_vector(
        "high_energy", model_output.get("high_energy"), len(low)
    )
    # A relative percentile cannot establish silence. Require real dynamic
    # range and a reduction in positive voice-band energy, not bass subtraction.
    raw_activity = mid + 0.35 * high
    if len(raw_activity) < 16 or np.any(raw_activity < 0):
        return []
    floor, ceiling = np.percentile(raw_activity, [1, 99])
    if ceiling <= 1e-8 or ceiling - floor <= max(1e-6, ceiling * 0.20):
        return []
    activity = np.clip(raw_activity / ceiling, 0.0, 1.0)
    if len(activity) >= 3:
        activity = np.convolve(activity, np.ones(3) / 3.0, mode="same")
    fps = contract.MODEL_FPS
    candidates = []
    for center_ms in risky:
        ...
        context_level = float(np.quantile(local, 0.80))
        if context_level < 0.20 or float(np.ptp(local)) < 0.20:
            continue
        quiet_limit = min(0.20, context_level * 0.25)
        guard_limit = min(0.35, context_level * 0.40)
        ...
        for offset, is_quiet in enumerate(np.append(mask, False)):
            if is_quiet and run_start is None:
                ...
                run_end = offset
                if run_end - run_start >= 16:
                    absolute_start = begin + run_start + 4
                    absolute_end = begin + run_end - 4
                    guarded_start = absolute_start - 4
                    guarded_end = absolute_end + 4
                    if np.all(
                        activity[guarded_start:absolute_start] <= guard_limit
                    ) and np.all(activity[absolute_end:guarded_end] <= guard_limit):
                        candidates.append(
                            {
                                "start_ms": int(round(absolute_start * 1000 / fps)),
                                "end_ms": int(round(absolute_end * 1000 / fps)),
                                "cut_ms": int(
                                    round((absolute_start + absolute_end) * 500 / fps)
                                ),
                                "stable_minimum_ms": int(
                                    round((absolute_end - absolute_start) * 1000 / fps)
                                ),
                                "guard_ms": 80,
...
def _safe_at(position_ms, vocal_risk, safe_intervals):
    ...
    if vocal_risk.get("calibration", {}).get("cuts_authorized") is not True:
        return False, risk, "cuts-not-authorized"
...
def _band_windows(model_output, regions):
    vectors = {
        name: evidence.finite_vector(name, model_output.get(name))
        for name in ("low_energy", "mid_energy", "high_energy")
    }
    output = []
    for region_index, region in enumerate(regions):
        start = max(0, int(round(region["start_ms"] * contract.MODEL_FPS / 1000)))
        end = min(
            len(vectors["low_energy"]),
            int(round(region["end_ms"] * contract.MODEL_FPS / 1000)) + 1,
        )
        if end <= start:
            continue
        for frame in range(
            ...
            * max(
                ...
                int(
                    round(
                        contract.MODEL_FPS * 60 / max(region["normalized_tempo_bpm"], 1)
                    )
                ),
            ),
        ):
                ...
                + max(
                    ...
                    int(
                        round(
                            ...
                            * contract.MODEL_FPS
                            * 60
                            / max(region["normalized_tempo_bpm"], 1)
                        )
                    ),
                ),
            )
            output.append(
                {
                    "start_ms": int(round(frame * 1000 / contract.MODEL_FPS)),
                    "end_ms": int(round(frame_end * 1000 / contract.MODEL_FPS)),
    ...
    return _cover_timeline(output, 256)


def _structural_and_cues(
    model_output, beat_frames, regions, vocal_risk, safe_intervals, duration_ms
):
    energy = _normalized(evidence.finite_vector("energy", model_output.get("energy")))
    flux = _normalized(
        evidence.finite_vector(
            "spectral_flux", model_output.get("spectral_flux"), len(energy)
        )
    )
    low = _normalized(
        evidence.finite_vector(
            "low_energy", model_output.get("low_energy"), len(energy)
        )
    )
    mid = _normalized(
        evidence.finite_vector(
            "mid_energy", model_output.get("mid_energy"), len(energy)
        )
    )
    high = _normalized(
        evidence.finite_vector(
            "high_energy", model_output.get("high_energy"), len(energy)
        )
    )
    boundaries = []
    ...
    for region_index, region in enumerate(regions):
        ...
        downbeat_indices = [
            index for index in range(start, end + 1) if index % 4 == phase
        ]
        ...
        for beat_index in downbeat_indices:
            ...
            supported = [
                bars
                for bars in (4, 8, 16)
                if bars_from_origin > 0 and bars_from_origin % bars == 0
            ]
            if not supported:
                continue
            frame = beat_frames[beat_index]
            position_ms = int(round(frame * 1000 / contract.MODEL_FPS))
            key = (position_ms, region_index)
            if key in seen:
                continue
            seen.add(key)
            radius = max(4, contract.MODEL_FPS)
            ...
            novelty = float(
                np.max(
                    flux[
                        max(0, frame - radius // 2) : min(
                            len(flux), frame + radius // 2 + 1
                        )
                    ]
                )
            )
            spectral_change = float(
                np.mean(
                    [
                        abs(
                            np.mean(vector[frame : min(len(vector), frame + radius)])
                            - np.mean(vector[max(0, frame - radius) : frame])
                        )
            ...
            recurrence = max(
                0.0, 1.0 - abs(float(energy[frame]) - float(energy[max(0, frame - 1)]))
            )
            ...
            section_ms = int(
                round(16 * 4 * 60_000 / max(region["normalized_tempo_bpm"], 1))
            )
            evidence_score = min(
                1.0,
            ...
            if progress <= 0.35 and common["complete_section_after"]:
                cues_by_role["entry"].append(
                    {
                        **common,
                        "role": "entry",
                        "role_score": round(
                            evidence_score + (0.35 - progress) * 0.2, 8
                        ),
                    }
                )
            if progress >= 0.65 and common["complete_section_before"]:
                cues_by_role["exit"].append(
                    {
                        **common,
                        "role": "exit",
                        "role_score": round(
                            evidence_score + (progress - 0.65) * 0.2, 8
                        ),
                    }
                )
            if slope > 0.08 and common["complete_section_after"]:
                cues_by_role["drop"].append(
                    {
                        **common,
                        "role": "drop",
                        "role_score": round(evidence_score + min(0.25, slope), 8),
                    }
                )
            if slope < -0.06 and common["complete_section_before"]:
                cues_by_role["breakdown"].append(
                    {
                        **common,
                        "role": "breakdown",
                        "role_score": round(evidence_score + min(0.25, -slope), 8),
            ...
            if speech_safe and common["complete_section_before"]:
                cues_by_role["cut"].append(
                    {
                        **common,
                        "role": "cut",
                        "role_score": round(evidence_score + 0.1, 8),
                    }
                )
                    ...
                    cues_by_role["loop"].append(
                        {
                            **common,
                            "role": "loop",
                            "role_score": round(
                                evidence_score + 0.08 * (1.0 - seam_error), 8
                            ),
                            "loop_start_ms": int(
                                round(loop_start_frame * 1000 / contract.MODEL_FPS)
                            ),
                            "loop_end_ms": position_ms,
                            "loop_bars": 4,
                            "loop_seam_error": round(seam_error, 8),
                            "loop_verified": False,
                            "loop_verification": "candidate-energy-only",
                        }
                    )
    cues = []
    rankings = {}
    for role, values in cues_by_role.items():
        ...
        # A boundary shared by overlapping regions is one option. Preserve
        # every contributing region while keeping the strongest role evidence.
        merged = {}
        for value in values:
            timestamp = value["timestamp_ms"]
            if timestamp not in merged:
                merged[timestamp] = {**value, "supporting_region_indices": []}
            merged[timestamp]["supporting_region_indices"].append(value["region_index"])
        selected = list(merged.values())[:MAX_CUES_PER_ROLE]
    ...
    for item in boundaries:
        position = item["position_ms"]
        current = unique_boundaries.get(position)
        if current is None:
            unique_boundaries[position] = {
                **item,
                "supporting_region_indices": [item["region_index"]],
            }
        else:
            current["supporting_region_indices"].append(item["region_index"])
            current["bar_lengths"] = sorted(
                set(current["bar_lengths"] + item["bar_lengths"])
            )
    boundaries = sorted(
        unique_boundaries.values(), key=lambda item: item["position_ms"]
    )
    return (
        _cover_timeline(boundaries, 128),
        cues[: MAX_CUES_PER_ROLE * len(CUE_ROLES)],
        rankings,
    )
...
def build_dj_analysis_v3(
    ...
    *,
    ...
    beat_logits = evidence.finite_vector("beat_logits", model_output.get("beat_logits"))
    downbeat_logits = evidence.finite_vector(
        "downbeat_logits", model_output.get("downbeat_logits"), len(beat_logits)
    )
    beat_frames = evidence.peak_frames(beat_logits)
    raw_downbeat_frames = evidence.peak_frames(downbeat_logits)
    matched = evidence.match_downbeats(
        beat_frames, raw_downbeat_frames, contract.MODEL_FPS
    )
    regions = _localized_regions(
        beat_frames, matched, beat_logits, downbeat_logits, contract.MODEL_FPS
    )
    duration_ms = int(round(duration_seconds * 1000))
    vocal_risk = evidence.vocal_risk_timeline(
        vocal_output, duration_seconds, vocal_calibration
    )
    safe_intervals = _speech_safe_intervals(model_output, vocal_risk, duration_ms)
    ...
    natural_entry = (
        int(round(int(audible[0]) * 1000 / contract.MODEL_FPS)) if len(audible) else 0
    )
    natural_exit = (
        int(round(int(audible[-1]) * 1000 / contract.MODEL_FPS))
        if len(audible)
        else duration_ms
    )
    payload = {
        ...
        "beats": {
            "timeline": [
                {
                    "position_ms": int(round(frame * 1000 / contract.MODEL_FPS)),
                    "confidence": round(evidence.sigmoid(beat_logits[frame]), 8),
                }
            ...
            "downbeats": [
                {
                    "position_ms": int(
                        round(item["snapped_frame"] * 1000 / contract.MODEL_FPS)
                    ),
                    "raw_position_ms": int(
                        round(item["raw_frame"] * 1000 / contract.MODEL_FPS)
                    ),
                    "confidence": round(
                        evidence.sigmoid(downbeat_logits[item["raw_frame"]]), 8
                    ),
        ...
        "natural_boundaries": {
            "entry_ms": 0,
            "exit_ms": duration_ms,
            "confidence": "source-timeline",
        },
        "suggested_audible_boundaries": {
            "entry_ms": natural_entry,
            "exit_ms": min(duration_ms, natural_exit),
            "confidence": "model-envelope",
            "trim_authorized": False,
        },
        "key_evidence": evidence.key_evidence(key_evidence),
        "quality": {
            "high_region_count": sum(
                item["confidence_tier"] == "high" for item in regions
            ),
            "medium_region_count": sum(
                item["confidence_tier"] == "medium" for item in regions
            ),
            "unavailable_region_count": sum(
                item["confidence_tier"] == "unavailable" for item in regions
            ),
            "cue_counts": {
                role: sum(item["role"] == role for item in cues) for role in CUE_ROLES
            },
            "beat_count": len(beat_frames),
            "raw_downbeat_count": len(raw_downbeat_frames),
            "speech_safe_interval_count": len(safe_intervals),
            "vocal_calibration_ready": vocal_risk.get("calibration", {}).get("status")
            == "ready",
        },
    }
    payload["analysis_digest"] = evidence.analysis_digest(payload)
    return payload
...
def analyze_dj_file_v3(
    ...
    *,
    ...
    adapter=None,
    yamnet_adapter=None,
    deadline_seconds=contract.JOB_DEADLINE_SECONDS,
    cancelled=None,
    progress=None,
    key_evidence=None,
    vocal_calibration=None,
