...
| `isolation_bench.py [--files 20] [--typical-seconds 240]` | File analysis in the LUM-018 worker process (P3-8): the worker's start, the median per-file overhead against in-process analysis (waveform and edge, synthesized FLAC), a typical track both ways, and the worker's RSS. Needs `av`, no database. | — |
