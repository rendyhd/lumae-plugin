...
| `isolation_bench.py [--files 20] [--typical-seconds 240] [--repeats 7]` | File analysis in the LUM-018 child process (P3-8), on the fork path and the exec path, against in-process analysis. Host-like: each job runs in a fresh fork of a parent that has the analyzers imported, as AudioMuse runs jobs (a 1-track edge job, a 3-track waveform job; median job time and overhead per job and per file). Also the per-file overhead in a long-lived process and the exec worker's start and RSS. Synthesized FLAC; needs `av`, no database. | — |
