...
| `lum005_k6` | A full waveform republish reaches a device at <= 1 KB per track with K6 edge references (P3-2): the device opts in, keeps every matching edge, and fetches the edges it lacks (`--lum005-misses`, default 50 dropped local edges) through `/api/profiles` (C-10). A second device without K6 (`edge_refs=False`) must still receive every edge in full; both end equal to the server. Against a plugin without `capabilities.profile_stream.edge_refs` it measures the no-K6 baseline on `--lum005-sample` tracks and reports PENDING. |
