...
5. **Do not re-send unchanged edges.** *(Corrected 2026-09-24, P0-5.)* A waveform republish does **not** embed the edge: `complete_attempt` deletes the edge rows server-side (`profile_publication.py:342-359`) and emits an upsert without `edge_profile`, and clients then delete their copy because an upsert without an edge removes it. The fix is P1-1 (keep the edge when the media is unchanged and embed it in the upsert) followed by K6/P3-2 (send an `edge_profile_ref` instead of the full edge to opted-in clients). A future analyzer change such as LUM-005 v2 then costs about 0.5 KB per track instead of about 19 KB.
