...
def main():
    ...
    with app.app_context():
        ...
        with db.cursor() as cur:
            ...
            elif (existing_provider[0] != "navidrome"
                  or not isinstance(existing_provider[1], dict)
                  or any(existing_provider[1].get(key) != value
                         for key, value in expected_provider.items())
                  or any(existing_provider[1].get(key) not in (None, "")
                         for key in set(existing_provider[1]) - set(expected_provider))):
