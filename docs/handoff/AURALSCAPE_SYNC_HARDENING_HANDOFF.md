...
- **Server note (plugin P3-4a, K8 implemented in 1.3.0, unreleased; contract §5.2, §5.2a, §5.3).**
  ...
  - Restores above 2,000 rows commit in chunks. Send an `Idempotency-Key`: a retry with the same key and body resumes the restore. Final revisions can exceed 2, so use the returned revision.
