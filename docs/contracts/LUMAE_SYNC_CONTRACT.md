...
| `integrity` | absent | **New in 1.3.0 (unreleased, P1-3, AUD-05).** Operator diagnostics, not a client gate: `{collections_feed_ok: bool\|null, profiles_unpublished_ready: int\|null, profiles_orphaned: int\|null, profiles_checked_at: string\|null, fences_installed: bool\|null}` (`profiles_orphaned` from P3-7). `fences_installed` is live (one catalogue lookup): `false` means the 1.3.0 migration has not installed every old-writer fence (`writer_generation` NOT NULL without default on `profile_changes` and `catalog_changes`, no default on `collection_changes.seq`). `collections_feed_ok` is live: `false` means a collection change row sits past the feed head, and every collection mutation then returns 503 `collection_feed_invariant` (§5.3) until an operator repairs it. `profiles_unpublished_ready` counts current `ready` source profiles with no published row, and `profiles_orphaned` published profiles whose track the source's current catalogue generation no longer has; both are recounted at install, at web-worker start and by the settings repair (P3-7), and `profiles_checked_at` (UTC, `Z`) says when. `null` means unknown (no database or an older schema). Repair SQL: `docs/runbooks/UPGRADE_1.3.md`. |
...
- From 1.3.0 (P3-10, LUM-021) the free-text error fields are redacted and capped at 300 characters: whitespace is collapsed, so a multi-line error becomes one line, and credentials, tokens and file paths are masked. The fields are `servers[].catalog.last_error`, `servers[].analysis.last_error`, `servers[].preparation.last_error` and `servers[].provider_identity_transition.last_error`, plus `reason` of a 503 `server_discovery_failed`; `last_error` of `/api/catalog/prepare` is treated the same way. Field names and types are unchanged, a retry category code is returned as is, and structured codes (`status`, `phase`, `refresh_reason`, `detection_reason`, `required_action`) are untouched.
...
- `failed`: the latest attempt is `failed` or `skipped_no_file`, with `last_error` as the reason, or serialization failed.
  ...
  - From 1.3.0 (P3-8, LUM-018) the server records two finer categories, `media_error` (the decoder rejected the data) and `analysis_crash` (the analysis worker died). It reports them as `unsupported_media` and `analysis_error`, so no new reason reaches clients (§8.5).
  - The per-file time limit is the server setting `analysis_time_limit_seconds` (default 900 s, the fixed deadline of 1.2.5). An analysis still running at the limit stops, and a hung one is killed 30 s later. Both are reported as `analysis_timeout`, as before.
...
- **`POST /api/profiles/edges/backfill`** (`__init__.py:2810-2826`). Body: `{catalog_instance_id?, after?: string ≤512, limit?: int 1–100}`.
  ...
  - Candidates are taken in track-ID order after `after`: tracks without an edge for their media and without a job attempted in the last 6 hours; `next_after` is the last candidate. From P3-7 (unreleased, ships in 1.3.0) they are published waveform rows, as for analyze. Before, they were analysis-attempt rows, so a page could name tracks without a published row (which analyze then skipped) and miss published ones whose attempt was on other media.
...
- **K10 (1.3.0, unreleased, P3-5a; gate `capabilities.collections.source_scoped_items: true`).** Every item object (item and batch responses, collection detail, feed `upsert` payloads, snapshot, backups) adds **`catalog_instance_id`**: the catalogue the item was chosen from, or `null` when unknown.
  - **Write:** item bodies may send `catalog_instance_id` (a string of at most 256 characters; empty means absent; anything else is 400). Absent: an existing item id keeps its stored catalogue; a new one gets the install's only catalogue when exactly one has ever existed, else `null`. The response and the feed payload carry the value stored.
  - **Membership (K9)** is per catalogue: the same track, provider album id or album key may appear once per catalogue in a collection, and once more as `null`. Two `null` items with the same key still conflict, as in 1.2.5 (unique indexes over `COALESCE(catalog_instance_id, '')`).
  - **Upgrade:** existing items are scoped only when exactly one catalogue has ever existed (sources are never deleted); otherwise they stay `null`. An unscoped item whose scoped twin already exists stays `null`. The backfill writes no feed events, so a client that sees `source_scoped_items` for the first time should take one snapshot (§5.2a) to learn the stored values.

### 5.1a Workbench library reads (K10, 1.3.0, unreleased, P3-5a)

`GET /api/collections/library` (browse), `/library/stats`, `/library/album`, `/library/stream/<item_id>`, `/library/art/<item_id>` and `GET /api/collections/search` take the query parameter **`catalog_instance_id`**.

| Request | Answer |
|---|---|
| names an active source | that catalogue only; stream and art use the source row's `provider_type`, not the host's `MEDIASERVER_TYPE` |
| names no active source | 404 `{"error":"catalog_instance_not_found"}` (stream and art included) |
| absent or empty, exactly one active source | that source: a single-source client gets 1.2.5's bodies plus the echo below |
| absent or empty, more than one active source | **400 `{"error":"catalog_instance_required","catalogs":[{catalog_instance_id, provider_type, server_name, is_default}]}`** |
| absent or empty, no active source | empty results, as before; stream and art 404 |

- Browse adds a top-level `catalog_instance_id` (the catalogue served). Album detail adds a top-level `catalog_instance_id` (its tracks' catalogue, else the one requested, else `null`) and one per track. Stats and search bodies are unchanged.
- A browse query shorter than three characters is answered before any database work and is not checked.
- LUM-015: the workbench no longer offers the "newest year" sort (no year is published yet). `sort=year` is still accepted and orders as before.
- **LUM-014 (1.3.0, unreleased, P3-5b): albums are catalogue albums, `(catalog_instance_id, album_id)`.**
  - **Browse** (`scope=albums` and the albums section of `scope=all`) lists one row per `catalog_albums` row of the served catalogue's current generation, grouped by `album_id`. Same-name editions with distinct ids are separate rows. A row is `{kind:"album", title, artist, album_key, provider_album_id, cover_item_id, track_count, year, rating}`: **`provider_album_id` is always the catalogue `album_id`** (1.2.5 sent `null`). `artist` is the catalogue album's artist, else its tracks'. Rows sort by the old keys, then `album_id`. `GET /api/collections/search?kind=album` rows likewise add `provider_album_id` and are one per album.
  - `album_key` (`casefold(artist)::casefold(title)`) is kept for display and legacy callers only; it is not an identity and can be shared by several albums.
  - Stats `album_count` and the artists scope's `album_count` count distinct `album_id`s (1.2.5 counted distinct title and artist pairs, so editions were merged).
  - **Details:** `GET /library/album?provider_album_id=<album_id>&catalog_instance_id=…` is the lookup; `title` and `artist` are then optional. The body is unchanged in shape: `album.title` and `album.artist` come from the catalogue, `album.provider_album_id` is the id, and the tracks are that album's only. An id with no tracks in that catalogue answers 200 with `tracks: []`.
  - **Legacy fallback:** without `provider_album_id`, `title` and `artist` (the `album_key` a 1.2.5 client has) are still accepted: they are matched case-insensitively, and when several albums match (editions), the one with the lowest `album_id` answers, with its `provider_album_id` in the body. Nothing matching gives 1.2.5's empty body. Without an id, missing `title` or `artist` is still 400 `Album title and artist are required`.
...
- The journal (`collection_changes`) is never compacted; that waits until clients are observed on K8 (`floor_seq`). Idempotency receipts **are** now bounded (F2, 1.3.0, unreleased; see §5.3).
...
- **Receipts are not kept forever (F2, 1.3.0, unreleased).** A cron task deletes `collection_mutations` rows older than `RECEIPT_RETENTION_DAYS` (30) and `shelf_mutations` rows older than its own `RECEIPT_RETENTION_DAYS` (30), in bounded batches (`collection_manager.py`, `shelves.py`). Once a receipt is gone, a retry that reuses its key finds nothing: it is not a replay, and not a conflict — it runs again and is treated as a new request, under that key, as if the key had never been used. A client that only ever retries promptly after a response is unaffected; one that replays a key after 30 days of silence gets a second application of the request instead of the first one's stored answer.
...
2. **`delete` removes the profile and its edge** (`profile_publication.py:64-77`). From P3-7 (unreleased, ships in 1.3.0) a `delete` can also come from the orphan withdrawal: a published profile whose track the current catalogue generation no longer has (the 1.2.5 upgrade seeded such profiles for tracks 1.2.5 had dropped), withdrawn at install, after each catalogue refresh or by the settings repair. The same repair republishes a current `ready` profile that had no published row with an `upsert` without an edge, like any first publication (rule 1). Both are ordinary events; no client change.
...
| K10 | Collection items carry `catalog_instance_id` (LUM-013, additive); workbench routes take an explicit catalogue | P3-5 | Store and scope items (C-13) | `capabilities.collections.source_scoped_items:true` | 3 | shipped in 1.3.0 (unreleased, P3-5a): items carry `catalog_instance_id` (string or `null`) in routes, feed payloads, snapshot and backups; membership is unique per catalogue; workbench reads take `catalog_instance_id`, default to the only active source, and answer 400 `catalog_instance_required` with several (§5.1, §5.1a) |
...
12. **Collections today** (the baseline for K8/K9):
    ...
    - journal and receipts are never compacted; **1.3.0 (F2)** bounds the receipts and stale restore progress (§5.3), the journal still is not.
