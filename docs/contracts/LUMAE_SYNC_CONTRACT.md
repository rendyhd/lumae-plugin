...
- **503 `{"error":"collection_feed_unavailable"}` with `Retry-After: 5`**: the feed state row is missing or has an unknown protocol, `DATABASE_URL` is unset, or the database is unreachable or timed out (statement timeout 30 s, lock timeout 5 s). A connection failure is logged as a warning naming only the error class.

**Size.** One response, not paged. Measured on the test database with realistic item rows: 20,000 items take 0.38 s and 8.0 MB of JSON (0.7 MB with K1 gzip); 100,000 items (the backup limit) take 1.6 s and 40 MB (3.7 MB gzip), with about 200 MB of transient Python memory in the web worker. At household sizes no page shape is needed, so none is defined. If one is needed later it would be additive, for example `?limit=&after=<opaque>` pages that each run their own REPEATABLE READ read and return `has_more`, with the client continuing the feed from the **first** page's `head_seq`: every event carries the entity's full state or a delete by id, so replaying events a later page already reflects is harmless.
