...
| P3-10 LUM-021 diagnostics closure | review pending | Phase 3 PR | | 922 passed, 1 xfailed; `/database-state`: stored `last_error` shown through the new `redaction.py` (DSN/URL userinfo, Subsonic t/s/p and secret query params, Bearer/Basic/cookie headers, key=value secrets, JWT/API-key/long tokens, file paths; retry codes unchanged; 300-char cap); every read in a savepoint under `SET LOCAL statement_timeout = 3000` (slowest read 0.34–0.39 s p50–p95 at scale 1, page ~1 s), rolled back so the host timeout and transaction are unchanged; a failed or timed-out section shows "unavailable", logged by class; waveform counts are published profiles plus 9 work states mirroring `fetch_backfill_rows` (equivalence test on every state) and failure categories from `profile_publication`; 8 mutations each caught |
