...
| P3-8 LUM-018 execution limits | review pending | Phase 3 PR | | 931 passed, 1 xfailed; each waveform/edge analysis runs in a pooled subprocess worker; `analysis_time_limit_seconds` (default 900) is the analyzers' soft deadline, passed per file (no worker restart), and the worker is killed 30 s after it (SIGTERM, SIGKILL after 5 s); new categories `media_error` (revision) and `analysis_crash` (transient), SQL retry selection takes the Python set; safe diagnostics in `source_profiles.failure_diagnostics` and the edge job reason; worker start 0.9–1.1 s per job, warm per-file overhead 1.1–2.0 ms median (20 × 5 s FLAC), 4-min track in-worker 0.876 s vs in-process 0.878 s, worker RSS 124 MB idle / 255–264 MB peak; 14 mutants caught (kill, SIGKILL, crash/timeout/InvalidData misclassified, SQL list, category set, close_fds, select for poll, path leak, deadline not passed, fixed 900 s deadline, kill without headroom) |
