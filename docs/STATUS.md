...
| P3-4a collections feed epoch, snapshot, bounded writes | review pending | — | — | 874 passed, 1 xfailed; K8: feed adds `epoch`/`head_seq`/`floor_seq`/`has_more`, 410 `collections_resync_required` only with an echoed `epoch` (old-client transcript byte-identical to the f36e087 golden), `GET /api/collections/snapshot` in one owned REPEATABLE READ read (20k items 0.38 s, 8.0 MB / 0.7 MB gzip); events written as one seq block per mutation; 3 s lock bound → 503 `collection_busy` + `Retry-After: 5`; restores in resumable ≤2,000-row chunks. 20k restore: feed head held 17.6 s in one hold (other principal's PATCH timed out at 4.0 s) → 11 holds of ≤58 ms, other principal's write max 56 ms; restore 32.3 → 6.7 s; mutations caught: 410 without echo, no lock_timeout, snapshot in READ COMMITTED, block off by one (both ways) |
