...
| P3-13 LUM-001 concurrency test strength | review pending | Phase 3 PR | | 873 passed, 1 xfailed; 14 tests cover both append paths (record_profile_change, record_profile_deletions) and compaction; 7 lock mutants each fail 4–14 of them (helper without `FOR UPDATE`: 14, before: 1 of 705); wait probe stable in 40 runs under 6 CPU burners |
