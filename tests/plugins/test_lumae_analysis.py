...
def test_plugin_manifest_has_lumae_identity():
    ...
    assert manifest["versions"][0]["version"] == "1.2.2"
...
def test_health_endpoint_reports_schema_and_analyzer_versions(monkeypatch):
    ...
    assert response.get_json() == {
        ...
        "analyzer_version": 1,
        "capabilities": {
            "personal_discovery": {"schema_version": 1, "enabled": False, "scope": "shared"},
...
def test_catalog_health_exposes_persisted_v3_0_3_source_readiness(monkeypatch):
    ...
    assert response.status_code == 200
    body = response.get_json()
    assert body["plugin_version"] == "1.2.2"
...
# Relationship publication is exercised against PostgreSQL in test_relationship_build.py.
